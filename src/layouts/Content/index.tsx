...
> = (props) => {
  const { style, tabPaneList, setTabPaneList, activeKey, setActiveKey } = props;
  ...
  const remove = (key: string) => {
    const newTabPaneList = tabPaneList.filter(
      (tabPane) => tabPane.key !== key || tabPane.key === DEFAULT_ACTIVE_KTY,
    );
  ...
  return (
    <ConfigProviderWrap>
      <ErrorComponent>
        <Layout.Content className={styles.content} style={style}>
              ...
              {tabPaneList.map((item) => {
                ...
                return (
                  <Tabs.TabPane
                    ...
                    forceRender
                  >
                    <div className={styles.layoutTabPane}>
